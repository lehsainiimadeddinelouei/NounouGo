                  ...
                  try {
                    ...
                    try {
                      await FirebaseFirestore.instance
                          .collection('deleted_accounts').doc(uid).set({
                        'uid':   uid,
                        'email': email,
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.circular(20)),
                          child: const Row(mainAxisSize: MainAxisSize.min, children: [
                            Icon(Icons.child_care_rounded, size: 14, color: Colors.white),
                            SizedBox(width: 6),
                            Text('Babysitter',
                        ...
                        if (score > 0)
                          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                            ...List.generate(5, (i) => Icon(
                              i < score.floor()
                                  ? Icons.star_rounded
                                  : (i < score ? Icons.star_half_rounded : Icons.star_outline_rounded),
              ...
              child: Column(
                  ...
                  children: [
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              gradient: const LinearGradient(
                                colors: [Color(0xFF7B9FFF), AppColors.buttonBlue],
                                begin: Alignment.topLeft, end: Alignment.bottomRight,
                              ),
                              borderRadius: BorderRadius.circular(14),
                              boxShadow: [BoxShadow(
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        return _ProfileGrid(
                          ...
                          bio: bio,
                          disponibilitesMap: disponibilitesMap,
                          disponibilitesFlat: disponibilitesFlat,
                          ageGroups: ageGroups,
                          diplomes: diplomes,
                    ...
                    _DashboardCard(
                      ...
                      title: 'Avis',
                      child: AvisNounouSection(nounouUid: widget.nounouData['uid'] ?? ''),
                    ),
...
class _DashboardCard extends StatelessWidget {
  ...
  final String title;
  final Widget child;
  const _DashboardCard({required this.icon, required this.title, required this.child});
          ...
          Row(children: [
            ...
            Text(title.toUpperCase(),
                style: const TextStyle(
                    ...
                    color: AppColors.textGrey, letterSpacing: 1.1)),
          ]),
          const SizedBox(height: 14),
          child,
        ],
...
class _ProfileGrid extends StatelessWidget {
  ...
  final String bio;
  final Map<String, List<String>> disponibilitesMap;
  final List<String> disponibilitesFlat;
  ...
  const _ProfileGrid({
    ...
    required this.bio,
    required this.disponibilitesMap,
    required this.disponibilitesFlat,
  ...
  // ── Green palette for disponibilités ──────────────────────
  static const Color _dispoGreen      = Color(0xFF2E7D32); // text / icon
  static const Color _dispoGreenLight = Color(0xFFE8F5E9); // chip bg
  static const Color _dispoGreenBorder= Color(0xFFA5D6A7); // chip border
  static const Color _dispoIconBg     = Color(0xFFC8E6C9); // card icon bg
  ...
  Widget build(BuildContext context) {
    ...
    // ── Disponibilités (GREEN) ────────────────────────────────
    if (disponibilitesMap.isNotEmpty) {
      cards.add(_DispoCard(
        icon: Icons.schedule_rounded,
        title: 'Disponibilités',
        iconBgColor: _dispoIconBg,
        iconColor: _dispoGreen,
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: disponibilitesMap.entries.toList().asMap().entries.map((outer) {
            final isLast  = outer.key == disponibilitesMap.entries.length - 1;
            ...
            return Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: periodes.map((p) => Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(
                      color: _dispoGreenLight,
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(color: _dispoGreenBorder),
                    ),
                    child: Text(p,
                        style: const TextStyle(
                            fontSize: 12, color: _dispoGreen, fontWeight: FontWeight.w600)),
                  )).toList(),
                ),
                if (!isLast) ...[
                  const SizedBox(height: 8),
      ...
      cards.add(_DispoCard(
        ...
        child: Wrap(
          ...
          children: disponibilitesFlat.map((d) => Container(
            ...
            child: Text(d,
                style: const TextStyle(
                    fontSize: 13, color: _dispoGreen, fontWeight: FontWeight.w600)),
          )).toList(),
        ),
      ));
    }
    ...
    if (ageGroups.isNotEmpty) {
      cards.add(_DashboardCard(
        ...
        child: Wrap(
          spacing: 8, runSpacing: 8,
          children: ageGroups.map((a) => _Tag(label: a, color: AppColors.primaryPink)).toList(),
        ),
      ));
    }
    ...
    if (diplomes.isNotEmpty) {
      cards.add(_DashboardCard(
        ...
        title: 'Diplômes',
        child: Column(
          children: diplomes.asMap().entries.map((e) => Column(children: [
            Row(children: [
              Container(
                width: 32, height: 32,
                decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.school_outlined, size: 16, color: AppColors.primaryPink),
              ),
              const SizedBox(width: 12),
              Expanded(child: Text(e.value,
                  style: const TextStyle(fontSize: 14, color: AppColors.textDark, fontWeight: FontWeight.w500))),
            ]),
            if (e.key < diplomes.length - 1) ...[
              const SizedBox(height: 8),
              const Divider(height: 1, thickness: 1, color: Color(0xFFF5EEF0)),
              const SizedBox(height: 8),
            ],
          ])).toList(),
        ),
    ...
    if (competences.isNotEmpty) {
      cards.add(_DashboardCard(
        ...
        child: Wrap(
          spacing: 8, runSpacing: 8,
          children: competences.map((c) => _Tag(label: c, color: AppColors.primaryPink)).toList(),
        ),
      ));
    }
    ...
    if (experiences.isNotEmpty) {
      cards.add(_DashboardCard(
        ...
        title: 'Expériences',
        child: Column(
          children: experiences.asMap().entries.map((e) => Column(children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                decoration: BoxDecoration(
                    color: AppColors.primaryPink.withOpacity(0.08),
                    borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.work_outline_rounded, size: 16, color: AppColors.primaryPink),
              ),
              const SizedBox(width: 12),
              Expanded(child: Text(e.value,
                  style: const TextStyle(
                      fontSize: 14, color: AppColors.textDark, fontWeight: FontWeight.w500, height: 1.4))),
            ]),
            if (e.key < experiences.length - 1) ...[
              const SizedBox(height: 8),
              const Divider(height: 1, thickness: 1, color: Color(0xFFF5EEF0)),
              const SizedBox(height: 8),
            ],
    ...
    for (int i = 0; i < cards.length; i++) {
      final padded = Padding(padding: const EdgeInsets.only(bottom: 12), child: cards[i]);
      if (i.isEven) { leftCol.add(padded); } else { rightCol.add(padded); }
    }
...
// ─────────────────────────────────────────────────────────────
// DISPO CARD — green-themed card wrapper (icon tinted green)
// ─────────────────────────────────────────────────────────────
class _DispoCard extends StatelessWidget {
  final IconData icon;
  final String title;
                ...
                children: [
                  Row(children: [
                    Container(
                      width: 36, height: 36,
                      decoration: BoxDecoration(
                        color: AppColors.primaryPink.withOpacity(0.1), shape: BoxShape.circle,
                      ),
                      child: Center(
                        child: Text(
                            auteur.isNotEmpty ? auteur[0].toUpperCase() : 'A',
